...
import { addInventoryExit, getActiveUserContext, getInventoryExits, getProducts } from '@/lib/mock/runtime-store'

export default function InventoryExitsPage() {
  const products = useMemo(() => getProducts(), [])
  const [records, setRecords] = useState(() => getInventoryExits())
  ...
  const itemOptions = useMemo(
    () => ['all', ...new Set(records.map((record) => record.product_name))],
    [records],
  )
  ...
  const filtered = useMemo(() => {
    ...
    return records.filter((record) => {
      ...
      const byItem = itemFilter === 'all' || record.product_name === itemFilter
  ...
  const registerExit = () => {
    ...
    const created = addInventoryExit({
      branch_id: branchId,
    ...
    setRecords(getInventoryExits())
    setQuantity('')
    setReason('')
    setSourceReference('')
    setFeedback(`Salida ${created.id} registrada correctamente.`)
  }
            ...
            {filtered.map((record) => (
              <div key={record.id} className="rounded-lg border border-zinc-800 bg-zinc-900/70 p-3 text-sm">
                <div className="flex flex-wrap items-center justify-between gap-2">
                  <p className="font-semibold text-zinc-100">{record.id} - {record.product_name}</p>
                  <Badge className="bg-amber-600 text-white">Salida</Badge>
                </div>
                <p className="text-zinc-400">
                  {new Date(record.created_at).toLocaleString('es-BO')} | {record.branch_name} | Usuario: {record.user_name}
                </p>
                ...
                <p className="text-zinc-400">Referencia: {record.source_reference || 'Sin referencia'} | Tipo: {record.source_type}</p>
              </div>
            ))}

            {filtered.length === 0 ? (
              <p className="text-sm text-zinc-400">No hay salidas para los filtros seleccionados.</p>
            ) : null}
          </CardContent>
        </Card>
      </div>
